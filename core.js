...
export function initSpecial(){
  ...
  function change(stat, delta){
    if(delta > 0){
      if(state.specialPool <= 0) return;
      if(state.special[stat] >= 10) return;
      state.special[stat] += 1;
      state.specialPool -= 1;
    } else {
      if(state.special[stat] <= 1) return;
      state.special[stat] -= 1;
      state.specialPool += 1;
    }
    render();
